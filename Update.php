...
$page = $_GET['page'];
if($page == "") { $page = 1;
echo '<font color="red">Đã Xong</font><br/>';
...
if($check > 0) {
$run = mysqli_query($apizophim,"UPDATE phim SET `ten_phim`='".$ten_phim."',`ten_goc`='".$ten_goc."',`thumb`='".$thumb."',`dien_vien`='".$dien_vien."',`thoi_luong`='".$thoi_luong."',`trang_thai`='".$trang_thai."',`time`='".($time-$no)."' where api='1' and slug='".$slug."'");
echo '<br/>--- Update : '.$ten_phim;
} else {
$az = mysqli_query($apizophim, "INSERT INTO `phim` (`slug`,`ten_phim`,`ten_goc`,`thumb`,`bole`,`nam_chieu`,`trang_thai`,`thoi_luong`,`quoc_gia`,`the_loai`,`dien_vien`,`noi_dung`,`api`,`hot`,`hide`,`view`,`view_day`,`time`) VALUES ('".$slug."','".$ten_phim."','".$ten_goc."','".$thumb."','".$bole."','".$nam_chieu."','".$trang_thai."','".$thoi_luong."','".$quoc_gia."','".$the_loai."','".$dien_vien."','".$noi_dung."','1','0','0','0','0','".($time-$no)."')");
